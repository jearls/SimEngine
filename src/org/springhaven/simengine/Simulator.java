...
/**
 * <p>The public interface to a simulation engine.</p>
 * 
 * <p>A simulation engine consists of:</p>
 * <ul>
 ...
 * </ul>
 * 
 * <p>In the <b>STOPPED</b> state, no work is done and the clock is not active.
 ...
 * 
 * <p>In the <b>STARTING</b> state, the clock and all workers are initialized.
 * In this state, no modifications can be done to the set of workers. When the
 * clock and all workers are initialized, the simulator transitions to the
 * <b>RUNNING</b> state.</p>
 * 
 * <p>In the <b>RUNNING</b> state, time is divided into <b>frames</b>. One frame
 ...
 * stopped) a new frame begins.</p>
 * 
 * <p>In this state, workers can be added, but will not be initialized until the
 * end of the current frame.</p>
 * 
 * <p>When there are no remaining workers, or when the simulator is stopped
 * externally (via the {@link #stop()} method), the simulator transitions to the
 * <b>STOPPING</b> state.</p>
 * 
 * <p>In the <b>STOPPING</b> state, all workers get cleaned up and removed from the
 * simulator and the clock gets reset; then the simulator transitions back to
 * the <b>STOPPED</b> state.</p>
 * 
...
public interface Simulator {
    /**
     * Returned by {@link #getState()} to indicate the simulator is in the <b>STOPPED</b> state. 
     */
    public final int STOPPED      = 0;
    /**
     * Returned by {@link #getState()} to indicate the simulator is in the <b>STARTING</b> state.
     */
    public final int STARTING = 1;
    /**
     * Returned by {@link #getState()} to indicate the simulator is in the <b>RUNNING</b> state.
     */
    public final int RUNNING      = 2;
    /**
     * Returned by {@link #getState()} to indicate the simulator is in the <b>STOPPING</b> state.
     */
    ...
    /**
     * @param name the name of the worker to return
     * @return the named worker
     * @throws NoSuchWorkerException if the named worker does not exist
     */
    public Worker getWorker(String name) throws NoSuchWorkerException;

    /**
     * @param name the name of the new worker
     * @param worker the worker to add to the simulation
     * @throws DuplicateWorkerException if the named worker already exists
     */
    public void addWorker(String name, Worker worker) throws DuplicateWorkerException;

    /**
     * @param name the name of the worker to remove
